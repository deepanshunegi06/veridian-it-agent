...
- Assignment data pack, transcribed into `backend/data/` without rewording policy text:
  - `backend/data/requests.yaml`: 15 employee requests, REQ-01 to REQ-15, with `employee`, `email`, `opened`, `text`, `initial_action`.
  - `backend/data/tickets.yaml`: 10 existing tickets, TK-1042 to TK-1051, with `summary`, `status`, `open`.
  - `backend/data/knowledge_base.yaml`: 11 clauses, KB-01 to KB-10 plus ASSET-01, each with `id`, `title`, `source`, `authority`, `text`, `keywords`.
...
- `MAX_FOLLOWUPS = 2` in `backend/app/tools.py` and `max_steps = 10` in `backend/app/llm.py` are chosen budgets, not supplied. `temperature = 0.1` is chosen for determinism.
...
- No real ticketing. `raise_ticket` persists to `backend/app/db.py` (Mongo `conversations`/`events`, in-memory fallback) with a global `TK-1052+` counter. No external system is called.
...
Session state is `ConversationStore` in `backend/app/main.py`, backed by `backend/app/db.py`.
...
- `unsupported_figure` misses hyphenated forms. `_NUMBERS` in `backend/app/tools.py` matches `<number><space><unit>` (e.g. `24 hours`) but not `4-year`, which is how ASSET-01 phrases the refresh cycle. A figure check against that clause can false-positive or pass vacuously depending on how the answer phrases it.
